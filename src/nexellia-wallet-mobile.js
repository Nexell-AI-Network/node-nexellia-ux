import {
	html, css, FlowFormat, NexelliaWalletUI, dpc,
	baseUrl, NXL, renderPagination, buildPagination, paginationStyle,
...
} from './nexellia-wallet-ui.js';
export {isMobile, dontInitiatedComponent};
export class NexelliaWalletMobile extends NexelliaWalletUI{
	...
	static get styles(){
		return [NexelliaWalletUI.styles, paginationStyle, swipeableStyle, css`
			:host{
			...
			.header{
				...
				padding:var(--nexellia-wallet-header-padding, 5px 10px);
				margin:var(--nexellia-wallet-header-margin, 0px);
			}
			...
			.pagination-box{
				padding:var(--nexellia-pagination-box-padding, 10px 5px;);
			}

			.logo{
				width:30px;height:30px;/*background-color:#DDD*/
				display:var(--nexellia-wallet-header-logo-diaplay, initial);
			}
			...
			.tabs-container{
				...
				border-top:2px solid var(--nexellia-wallet-tab-border-top-color, var(--flow-primary-color));
				border-bottom:1px solid var(--nexellia-wallet-tab-border-bottom-color, #DDD);
			}
			.tabs{
			...
			.tab.selected{
				border-bottom-color:var(--nexellia-wallet-tab-active-border-color, var(--flow-primary-color));
			}
			...
			.address-input{
				...
				min-width:var(--nexellia-wallet-address-input-min-width, 460px);
			...
			.send-scan-buttons a fa-icon{
				...
				background-color:var(--nexellia-wallet-scan-button-bg, #3d4e58);
				--fa-icon-color:var(--nexellia-wallet-scan-button-color, #FFF);
				box-shadow:var(--flow-box-shadow);
			}
			.send-scan-buttons .send-btn fa-icon{
				background-color:var(--nexellia-wallet-send-button-bg, #fb7470);
				--fa-icon-color:var(--nexellia-wallet-send-button-color, #FFF);
			}
			.send-scan-buttons .receive-btn fa-icon{
				background-color:var(--nexellia-wallet-receive-button-bg, #60b686);
				--fa-icon-color:var(--nexellia-wallet-receive-button-color, #FFF);
			}
			...
			.lock-screen{
				...
				position:absolute;
				z-index:1000000;
				background-color:var(--nexellia-lock-screen-bg-color);
				top:0px;bottom:0px;
				left:0px;right:0px;
			}
	...
	render(){
		...
		let donationAddresses = [
			...
			["Nexellia WebWallet donations:", "nexellia:qqe3p64wpjf5y27kxppxrgks298ge6lhu6ws7ndx4tswzj7c84qkjlrspcuxw"],
		]
		...
		<div class="tab-contents flow-swipeable-container" ?not-ready=${!isReady}>
			<div class="flow-swipeable-row">
				...
				<div class="tab-content ${sCls('wallet')}" for="wallet">
					<div class="wallet-ux">
						<div class="badge" is="i18n-div">NEXELLIA WALLET</div>
						...
						<flow-expandable class="donation-info" no-info no-icon icon="-">
							...
							<p is="i18n-p">
								if you wish to further the development of the nexellia ecosystem, we accept donations at the following addresses:
							</p>
							${
						...
						<flow-expandable class="developer-info" _expand no-info no-icon icon="-">
							...
							<div class="badge"><span is="i18n-span">Nexellia Core NXL:</span> ${window.PWA_MODULES['@nexellia/core-lib']}</div>
							<div class="badge"><span is="i18n-span">Nexellia Wallet Framework NXL:</span> ${window.PWA_MODULES['@nexellia/wallet']}</div>
							<div class="badge"><span is="i18n-span">Nexellia gRPC:</span> ${window.PWA_MODULES['@nexellia/grpc']}</div>
							<div class="badge"><span is="i18n-span">Nexellia gRPC Relay:</span> ${window.PWA_MODULES['@nexellia/grpc-web']}</div>
							<div class="badge"><span is="i18n-span">Nexellia UX NXL:</span> ${window.PWA_MODULES['@nexellia/ux']}</div>
							<div class="badge"><span is="i18n-span">Flow UX NXL:</span> ${window.PWA_MODULES['@aspectron/flow-ux']}</div>
						</flow-expandable>
				...
				<div class="tab-content ${sCls('faucet')}" for="faucet">
					${this.faucetStatus ? this.faucetStatus : html`
						...
						<div class="faucet-ux">
							<div class="margin-bottom"  is="i18n-div">NEXELLIA FAUCET</div>
							<div>${i18nFormat('Your IP is [n]', this.ip||"")}</div>
							<div class="margin">${i18nHTMLFormat('You have <b>[n] NXL</b> available.', NXL(this.faucetFundsAvailable||0) )}</div>
				...
				<div class="tab-content ${sCls('debuginfo')}" for="debuginfo">
					<div class="info-ux">
						...
						<table>
							...
							<tr><td is="i18n-td">AMOUNT</td><td>${NXL(inUseUTXOs.satoshis||0)} NXL</td></tr>
						</table>
						<flow-btn class="center-btn primary v-margin"
	...
	renderLockScreen(){
		return html`
			<div class="lock-screen">
				<div><img class="big-logo" src="/resources/images/nexellia.png"></div>
	...
	openAddressExplorer(address){
		if (!address)
			return
		let url = `https://explorer.nexell-ia.net/addresses/${address}`;
  			...
  			<div class="balance-badge">
				${ this.isLoading ? html`
					...
					<div class="balance pending">
						<span class="label-pending">${T('PLEASE WAIT')} <span class="dots">${this.dots}</span> ${total ? this.formatNXL(total)+' NXL':''}</span>
					</div>
				` : html`
					<div class="balance">
						<span class="value">${this.formatNXL(available)} NXL</span>
					</div>
					<div class="balance pending">
						<span class="label-pending" is="i18n-span">Pending:</span>
						<span class="value-pending">${this.formatNXL(pending)} NXL</span>
					</div>
				`}
            </div>
	...
	showReceiveDialog(){
		let address = this.receiveAddress||'nexelliatest:abc'
