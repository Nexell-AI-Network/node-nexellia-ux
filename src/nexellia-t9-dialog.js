...
import {
	html, css, NexelliaDialog, NXL,
	formatForMachine, formatForHuman
} from './nexellia-dialog.js';

class NexelliaT9Dialog extends NexelliaDialog{
	...
	static get styles(){
		return [NexelliaDialog.styles, css`
			.container{
				...
				max-height:var(--nexellia-dialog-container-max-height, 600px);
			}
	...
	renderBody(){
		...
		let {inputLabel='Amount in NXL'} = this;
		return html`
	...
	open(args, callback){
		...
		this.inputLabel = args.inputLabel||i18n.t('Amount in NXL');
		this.show();
	}
    cancel(){
    	this.hide();
...
NexelliaT9Dialog.define("nexellia-t9-dialog");
