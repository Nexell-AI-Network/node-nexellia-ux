...
export class NexelliaDialog extends BaseElement{
	...
	static get styles(){
		return [ScrollbarStyle, SpinnerStyle, css`
			:host{
				...
				position:var(--nexellia-dialog-position, absolute);
				top:0px;
				left:0px;
				width:100%;
				height:100%;
				background-color:var(--nexellia-dialog-bg-color);
			...
			.container{
				box-sizing:border-box;
				width:100%;
				height:var(--nexellia-dialog-container-height, calc(100% - 10px));
				background-color:var(--flow-background-color, #F00);
				z-index:1;
				border:var(--nexellia-dialog-container-border, 2px solid var(--flow-primary-color));
				border-radius:var(--nexellia-dialog-container-border-radius, 3px);
				max-width:var(--nexellia-dialog-container-max-width, 700px);
				max-height:var(--nexellia-dialog-container-max-height, 300px);
				margin:var(--nexellia-dialog-container-margin, 5px auto);
				padding:var(--nexellia-dialog-container-padding, 0px);
				position:relative;
			...
			:host([hideable]) .close-btn{
				display:var(--nexellia-dialog-container-close-btn-display, inline-block)
			}
			.heading{
			...
			[hidden]{display:none}
			.buttons{margin:var(--nexellia-dialog-buttons-margin, 10px auto);display:flex;width:var(--nexellia-dialog-buttons-width,90%);}
	...
	pushHistory(uid=""){
		...
		name = name.replace(/\-/g, "");
		let key = name.toLowerCase().replace(/(nexellia|dialog|mobile)/g, '')
