import {
	html, css, NexelliaDialog, NXL, txListStyle,
	paginationStyle, buildPagination,
	renderPagination, T
} from './nexellia-dialog.js';

class NexelliaTXDialog extends NexelliaDialog{
	...
	static get styles(){
		return [NexelliaDialog.styles, paginationStyle, txListStyle,
		css`
			:host{
				position:fixed;
			...
			.container{
				max-height:var(--nexellia-dialog-container-max-height, 90%);
				max-width:var(--nexellia-dialog-container-max-width, 90%)
			}
...
NexelliaTXDialog.define("nexellia-tx-dialog");
