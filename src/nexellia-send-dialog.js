...
import {
	html, css, NexelliaDialog, askForPassword, NXL,
	formatForMachine, getLocalWallet
} from './nexellia-dialog.js';
const pass = "";
import {Wallet} from '@nexellia/wallet-worker';

class NexelliaSendDialog extends NexelliaDialog{
	...
	static get styles(){
		return [NexelliaDialog.styles, 
		css`
			.container{
				max-height:var(--nexellia-dialog-container-max-height, 670px);
			}
	...
	renderBody(){
		return html`
			<flow-input class="address full-width" outer-border
				label="${T(`Recipient Address (Must start with 'nexellia' prefix)`)}"
				value="${this.address||''}"
				placeholder="">
			</flow-input>
			<div col>
				<flow-input class="amount full-width" outer-border
					label="${T('Amount in NXL')}" @keyup=${this.onAmountChange}
					value="${this.amount}">
				</flow-input>
				<div spacer></div>
	...
	renderEstimate(){
		...
		<div class="estimate-tx">
			<table>
				...
				${dataFee?html`<tr><td is="i18n-td">Data Fee</td><td>${NXL(dataFee)} NXL</td></tr>`:''}
				${fee?html`<tr><td is="i18n-td">Total Fee</td><td>${NXL(fee)} NXL</td></tr>`:''}
				${totalAmount?html`<tr is="i18n-td"><td>Total Amount</td><td> ${NXL(totalAmount)} NXL</td></tr>`:''}
			</table>
		</div>
		`
	}
    ...
    async sendAfterConfirming(){
    	...
    	if(estimate.fee > this.alertFeeAmount){
			let msg = i18n.t('Transaction Fee ([n] NXL) is too large.');
			msg = msg.replace('[n]', NXL(estimate.fee));
...
NexelliaSendDialog.define("nexellia-send-dialog");
