import {html, css, NexelliaDialog, i18n, T} from './nexellia-dialog.js';
import {askForPassword} from "./wallet.js";
const pass = "";

class NexelliaOpenDialog extends NexelliaDialog{
	...
	static get styles(){
		return [NexelliaDialog.styles, css`
			.container{max-height:var(--nexellia-dialog-container-max-height, 600px)}
			:host([mode="create"]) .container{max-height:var(--nexellia-dialog-container-max-height, 500px)}
			:host([mode="init"]) .container{max-height:var(--nexellia-dialog-container-max-height, 200px)}
			:host([mode="recover"]) .container{max-height:var(--nexellia-dialog-container-max-height, 450px)}
			.buttons{justify-content:center;--nexellia-dialog-buttons-width:100%;}
	...
	renderInitUI(){
		return html`
			<div class="sub-heading text-center" is="i18n-div">Welcome to Nexellia Wallet</div>
		`
	}
	renderRecoverUI(){
		let rows = [0, 1, 2];
	...
	renderOpenUI(){
		...
		return html`
			${this.hideLogo?'': html`
			<div>
				<img class="big-logo" src="/resources/images/nexellia.png" />
			</div>`}
	...
	_renderOpenButtons(){
		return html`
			<flow-btn primary @click="${e=>this.mode='create'}" i18n>NEW WALLET</flow-btn>
...
NexelliaOpenDialog.define("nexellia-open-dialog");
