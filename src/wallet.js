...
import {helper, Storage, CONFIRMATION_COUNT, COINBASE_CFM_COUNT} from '@nexellia/wallet-worker';
export const {Deferred, NXL, Decimal} = helper;
export {CONFIRMATION_COUNT, COINBASE_CFM_COUNT};
const storage = new Storage({logLevel:'debug'});
let {baseUrl, debug, MAX_UTXOS_THRESHOLD=80, dontInitiatedComponent=false} = window.NexelliaConfig || {};
if(!baseUrl){
	baseUrl = (new URL("../", import.meta.url)).href;
	debug && console.log("NexelliaUX: baseUrl", baseUrl)
}
...
/*
const {Wallet, initNexelliaFramework, Storage} = require("nexellia-wallet-worker");
let {Mnemonic} = Wallet;
...
export const {RPC} = require("nexellia-grpc-node");
*/
