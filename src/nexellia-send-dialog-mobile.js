...
class NexelliaSendDialogMobile extends NexelliaDialog{
	...
	renderEstimate(){
		...
		<div class="estimate-tx">
			<table>
				...
				${totalAmount?html`<tr><td is="i18n-td">Total Amount</td><td> ${NXL(totalAmount)} NXL</td></tr>`:''}
			</table>
		</div>
		`
	}
	...
	showT9(e){
		...
		showT9({
			value,
			heading:input.label.replace("in NXL", ""),
    ...
    async sendAfterConfirming(){
    	...
    	if(estimate.fee > this.alertFeeAmount){
			let msg = i18n.t('Transaction Fee ([n] NXL) is very large.');
...
NexelliaSendDialogMobile.define("nexellia-send-dialog-mobile");
